...
func (s *Service) initConfig() {
	c := new(conf)
	_, err := toml.DecodeFile("./config_zhihu/conf.toml", c)
	if err != nil {
		fmt.Println(err)
		panic(err)
	}
...
func (s *Service) initDB() {
	...
	db.AutoMigrate(&user{},&question{},&answer{},&article{},&comment{},&message{},&topic{},&topic_to_article{},&topic_to_question{},&token{})
}
